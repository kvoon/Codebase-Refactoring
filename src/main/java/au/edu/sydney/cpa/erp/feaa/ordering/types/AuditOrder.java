...
/**
 * Representation of an Audit order, moved over relevant existing code with slight modifications to
...
public class AuditOrder implements OrderType {
    ...
    /**
     * Default Constructor for Audit order types.
     */
    public AuditOrder() {
    ...
    public double getTotalCommission(Order order, PriorityType priorityType, ScheduleType scheduleType) {
        ...
        for (Report reports : order.getAllReports()) {

            cost += order.getReportEmployeeCount(reports) * reports.getCommission();
        }
    ...
    public String RegScheduledNonPriorityLongDesc(Order order, PriorityType priorityType, ScheduleType scheduleType, boolean finalised) {
        ...
        double totalLoadedCost = this.getTotalCommission(order, priorityType, scheduleType);
